...
class Edge:
    def __init__(self,length: int, node0: Node,
                node1: Node, direction=1, flag=None):
        self.length = length
        self.node0 = node0
        self.node1 = node1
...
class Graph:
    ...
    def get_outbound_adjacent_nodes(self,node: Node):
        ...
        for edge in edges:
            outbound_adjacent_nodes = outbound_adjacent_nodes.union(edge.get_destinies())
    ...
    def get_outbound_adjacent_nodes_with_flag(   self,node: Node,flag_name: str):
        return set([node for node in self.get_outbound_adjacent_nodes(node) if node.flag==flag_name])
    ...
    def get_in_bound_adjacent_nodes_with_flag(   self,node: Node,flag_name: str):
        return set([node for node in self.get_in_bound_adjacent_nodes(node) if node.flag==flag_name])
    ...
    def shortest_path_directional(self,n0,n1):
        ...
        while current_node is not n1:
            ...
            for n in adjacent_nodes:
                relevant_edges = [edge for edge in self.edges if edge.does_directionally_connect_nodes(current_node,n)]
                new_distance = current_node.distance + min([edge.length for edge in relevant_edges])
                n.distance = new_distance if n.distance == 'inf' else min(n.distance,new_distance)
            ...
            for node in connected_nodes:
                if node.flag == 'SEEN':
                    next_node = node if next_node is None else (next_node if next_node.distance <= node.distance else node)
